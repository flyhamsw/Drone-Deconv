...
def augment_image(original_img, sess):
    augmented_images = sess.run([img_flip_LR, img_flip_UD, img_flip_TR, img_rot_90, img_rot_180, img_rot_270], feed_dict={img: original_img})
    return augmented_images

def _bytes_feature(value):
    return tf.train.Feature(bytes_list=tf.train.BytesList(value=[value]))

def build_tfrecords(target_dataset, augment=False):
